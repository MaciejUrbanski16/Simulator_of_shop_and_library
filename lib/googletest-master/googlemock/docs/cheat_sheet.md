...
class Foo {
  ...
  virtual string Describe(const char* titleOfBooksInShop) = 0;
...
class MockFoo : public Foo {
  ...
  MOCK_METHOD(string, Describe, (const char* titleOfBooksInShop), (override));
...
in the chain a different titleOfBooksInShop. *All expected calls* in the same sequence must
...
The titleOfBooksInShop `seq` is irrelevant.
